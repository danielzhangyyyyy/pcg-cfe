<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_UI_NEWGROUPCOST"
...
<script>
    ...
    import {del, getDropDownList, list} from "@api/newGroupCost_api";
    import langZh from "../../locales/zh-CN/newGroupCost_lang.js";
    import langEn from "../../locales/en-US/newGroupCost_lang.js";
import { isNullOrUndefined } from 'util';
  ...
  watch: {
    ...
    familyDataList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "family") {
          if(this.familyDataList.length == 0){
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/newGroupCost/newGroupCost_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
    ...
    searchOnClick() {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          this.queryParam.family = values.family;
    ...
    getAllColumnSize() {
      ...
      this.tableSize.x = x + 100;
      console.log("total width", x, this.tableSize);
    },
    handleSubmit(e) {
      e.preventDefault();
      this.form.validateFields((err, values) => {
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/newGroupCost/newGroupCost_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("newGroupCost_editSelectKey", editSelectKey);
        this.$router.replace(`/newGroupCost/newGroupCost_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("newGroupCost_dblclickRowKey", rowKey);
      this.$router.replace("/newGroupCost/newGroupCost_detail");
