...
<script>
    ...
    import {getDropDownList, show} from "@api/newGroupCost_api";
  ...
  created() {
    this.changeLanguage();
    this.editSelectKey = Vue.ls.get("newGroupCost_editSelectKey");
  ...
  watch: {
    ...
    $route: {
      handler(route) {
        ...
        if (route.name === "newGroupCost_edit") {
          this.editSelectKey = Vue.ls.get("newGroupCost_editSelectKey");
          console.log(this.editSelectKey);
          this.getDetailfun(this.editSelectKey);
        }
        ...
        .then(res => {
          if (res.code != 0) {
            ...
            setTimeout(() => {
              ...
              this.$router.replace(`/newGroupCost/newGroupCost_list`);
            }, 500);
            return;
          }
