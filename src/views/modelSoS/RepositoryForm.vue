<template>
    <a-form @submit="handleSubmit" :form="form" class="form">
        <a-row class="form-row" :gutter="48">
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList" :key="item.index">
...
<script>
    ...
    import {getDropDownList, show} from '@api/modelSoS_api'
    ...
    export default {
        ...
        created() {
            this.changeLanguage()
            this.editSelectKey = Vue.ls.get('modelSoS_editSelectKey')
                ...
                show(rowKey).then(res => {
                    if (res.code != 0) {
                        ...
                        setTimeout(() => {
                            ...
                            this.$router.replace(`/modelSoS/modelSoS_list`);
                        }, 500);
                        return;
                    }
