<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="VAL_CPB_FREIGHT"
      exportId="GET_CPB_FREIGHT"
...
import { del, getDropDownList, list } from "@api/cpbFreight_api";
...
export default {
  ...
  watch: {
    ...
    buBrandDataList() {
      this.filterList.forEach(element => {
        if (element["decorator"] == "buGroup") {
          element["dropDownList"] = [].concat(this.buBrandDataList);
        }
      });
    },
    brandDataList(val) {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/cpbFreight/cpbFreight_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/cpbFreight/cpbFreight_add");
    },
    editSelectOne() {
      ...
      if (selectedRowKeyslen == 1) {
        if (this.historyStatus === "CURRENT") {
          ...
          Vue.ls.set("cpbFreight_editSelectKey", editSelectKey);
          this.$store.dispatch("ToggleCloseTab", "");
          setTimeout(() => {
            this.$store.dispatch(
              "ToggleCloseTab",
              "/cpbFreight/cpbFreight_edit" + this.lastQuery
            );
            this.lastQuery =encodeURI(
              "?cycle=" +
              this.selectedRows[0].cycle +
              "&buGroup=" +
              this.selectedRows[0].buGroup +
              "&brand=" +
              this.selectedRows[0].brand +
              "&family=" +
              this.selectedRows[0].family +
              "&subgeo=" +
              this.selectedRows[0].subgeo +
              "&country=" +
              this.selectedRows[0].country)
               
            this.$router.replace({
              path: "/cpbFreight/cpbFreight_edit",
              query: {
                cycle: this.selectedRows[0].cycle,
                buGroup: this.selectedRows[0].buGroup,
                brand: this.selectedRows[0].brand,
                family: this.selectedRows[0].family,
                subgeo: this.selectedRows[0].subgeo,
                country: this.selectedRows[0].country
              }
            });
          }, 500);
        } else {
    ...
    deleteSeclectAll() {
      ...
      this.selectedRows.forEach(element => {
        rowKeys.push({
          buGroup: element.buGroup,
          brand: element.brand,
          family: element.family,
          subgeo: element.subgeo,
