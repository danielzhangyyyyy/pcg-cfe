            ...
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in babelList" :key="index">
                <a-form-item :label="item.label">
                    <a-input
                            ...
                            v-if="item.inputType=='input'&& item.decorator === 'model'"
                    ...
                    <a-input
                            :placeholder="item.message"
                            v-if="item.inputType=='input'&&item.decorator === 'weight'"
                            :disabled="item.decorator=='productGroup'"
                            :class="{ plantUpperCase: item.decorator=='plant' }"
                            v-decorator="[item.decorator, {
                                        initialValue: item.text,
                                        rules: [{required: item.required, message: item.message,pattern: new RegExp(/^(\-|\+)?\d+(\.\d+)?$/) }]}]"
                    />
                </a-form-item>
            </a-col>
        </a-row>
...
<script>
    ...
    import Vue from "vue";
    import {getDropDownList} from "@api/outboundItem_api";
