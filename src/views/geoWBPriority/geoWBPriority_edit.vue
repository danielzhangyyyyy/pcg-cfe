...
<script>
    import Vue from "vue";
    import {update} from "@api/geoWBPriority_api";
  ...
  created() {
    this.editSelectKey = Vue.ls.get("geoWBPriority_editSelectKey");
  ...
  watch: {
    $route: {
      handler(route) {
        ...
        if (route.name === "geoWBPriority_edit") {
          this.editSelectKey = Vue.ls.get("geoWBPriority_editSelectKey");
          console.log(this.editSelectKey);
          ...
          .then(res => {
            if (res.code == 0) {
              ...
              setTimeout(() => {
                ...
                this.$router.replace(`/geoWBPriority/geoWBPriority_list`);
              }, 500);
            } else {
                 this.$notification.open({
                message: "Error",
