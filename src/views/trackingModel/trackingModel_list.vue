<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_TRACKING_MODEL"
...
import { list, del, getDropDownList } from "@api/trackingModel_api";
import { getXScrollSize } from "@api/publicFunc_api";
import langZh from "../../locales/zh-CN/trackingModel_lang.js";
import langEn from "../../locales/en-US/trackingModel_lang.js";
export default {
  name: "TableList",
  components: {
    STable,
  ...
  watch: {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/trackingModel/trackingModel_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
    ...
    searchOnClick() {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          this.queryParam.sbb = values.sbb
            ? "%" + values.sbb.trim() + "%"
            : values.sbb;
          this.queryParam.mtm = values.mtm
            ? "%" + values.mtm.trim() + "%"
            : values.mtm;
    ...
    handleDropDownChange(value, decorator) {
      ...
      if (decorator == "brand") {
        ...
        this.getDropDown(
          { moduleName: "getProdFamilyTMByBrand", brand: value },
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/trackingModel/trackingModel_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("trackingModel_editSelectKey", editSelectKey);
        this.$router.replace(`/trackingModel/trackingModel_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("trackingModel_dblclickRowKey", rowKey);
      this.$router.replace("/trackingModel/trackingModel_detail");
