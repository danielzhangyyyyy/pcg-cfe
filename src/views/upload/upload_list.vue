        ...
        methods: {
            customCellClick(record, dataIndex) {
                ...
                setTimeout(() => {
                    this.$store.dispatch(
                        "ToggleCloseTab",
                        "/upload/upload_detail" + this.lastQuery
                    );
                    this.lastQuery = encodeURI(
                        ...
                        "&eventName=" + eventName +
                        "&origin=" + origin
                    ).replace(new RegExp(/(:)/g), "%3A").replace(/\//g, '%2F');
            ...
            handleUpload() {
                ...
                // });
                let params = {}
                // eventName与rptId是flexReport上传需要的参数，不要删除
                if (this.$route.params.eventName == 'eventName') {
                    params = {eventName: this.$route.params.id}
                } else {
                    params = {
                        eventName: this.$route.params.eventName,
                        rptId: this.$route.params.rptId
                    }
                }
                upload(params, formData).then(res => {
                    // this.fileList = [];
                    this.uploading = false;
                    this.uploadDisabled = false;
