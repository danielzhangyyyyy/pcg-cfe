...
<script>
    import Vue from 'vue'
    import {update} from '@api/productDescription_api'
    ...
    export default {
        ...
        created() {
            this.editSelectKey = Vue.ls.get('productDescription_editSelectKey')
        },
        watch: {
            '$route': {
                handler(route) {
                    if (route.name === 'productDescription_edit') {
                        this.editSelectKey = Vue.ls.get('productDescription_editSelectKey')
                    }
                },
                deep: true
            ...
            validate() {
                ...
                const repositoryForm = new Promise((resolve, reject) => {
                    repository.form.validateFields((err, values) => {
                        ...
                        delete values.brand;
                        delete values.aspPrdFamily;
                        delete values.item;
                        delete values.sysLastModifiedBy;
                        delete values.sysLastModifiedDate;
                        resolve(values)
                    })
                })
                // this.loading = true;
                    ...
                    update(values[0]).then(res => {
                        if (res.code == 0) {
                            ...
                            setTimeout(() => {
                                ...
                                this.$router.replace(`/productDescription/productDescription_list`);
                            }, 500);
                        } else {
                            this.loading = false;
                            this.$notification.open({
