<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_COMPONENT_WARRANTY_DATA"
...
<script>
    ...
    import {del, getDropDownList, getTypeSelect, list} from "@api/componentWarranty_api";
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from "../../locales/zh-CN/componentWarranty_lang.js";
    import langEn from "../../locales/en-US/componentWarranty_lang.js";
  ...
  created() {
    ...
    this.getTypeSelect(this.typeDataList, "type");
  ...
  watch: {
    ...
    bizProdDataList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "prodFamily") {
          element["dropDownList"] = [].concat(this.bizProdDataList);
        }
      });
    },
    plantDataList(val) {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/componentWarranty/componentWarranty_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
    ...
    getTypeSelect(dropdownList, key) {
      getTypeSelect(Object.assign({})).then(res => {
        dropdownList.splice(0);
        res.result.forEach(el => {
          dropdownList.push(el[key]);
        });
    ...
    //     console.log(rowKey)
    //     Vue.ls.set('compoentWarranty_dblclickRowKey', rowKey)
    //     this.$router.replace('/compoentWarranty/compoentWarranty_detail')
    ...
    deleteSeclectAll() {
      ...
      this.selectedRowKeys.forEach(element => {
        console.log(element.split("@")[0], 11111);
        rowKeys.push({
          rid: element.split("@")[0]
        });
      });
