...
<script>
    import Vue from 'vue'
    import {update} from '@api/wbFilterEx_api'
    ...
    created () {
        this.editSelectKey = Vue.ls.get('wbFilterEx_editSelectKey')
        ...
        Promise.all([repositoryForm]).then(values => {
            ...
            update(values[0])
                ...
                this.$store.dispatch('ToggleCloseTab', this.$route.path);
                    this.$router.replace(`/wbFilterEx/wbFilterEx_list`)
                }, 500)
                // this.$emit('ok', values)
            })
        .catch(er => {
