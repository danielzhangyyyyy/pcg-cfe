<template>
    <a-form @submit="handleSubmit" :form="form" class="form">
        <a-row class="form-row" :gutter="48">
            ...
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList.slice(6,18)" :key="item.index">
                <a-form-item :label="item.label">
                    <a-input
                            :placeholder="item.message"
                            v-if="item.inputType=='input'"
...
<script>
    ...
    import {getDropDownList, show} from '@api/otherGSC_api'
    ...
    export default {
        ...
        created() {
            this.changeLanguage()
            this.editSelectKey = Vue.ls.get('otherGSC_editSelectKey')
                ...
                show(rowKey).then(res => {
                    if (res.code != 0) {
                        ...
                        setTimeout(() => {
                            ...
                            this.$router.replace(`/otherGSC/otherGSC_list`);
                        }, 500);
                        return;
                    }
