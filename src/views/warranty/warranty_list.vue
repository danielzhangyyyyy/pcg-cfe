...
<script>
    ...
    import {del, getDropDownList, list} from '@api/warranty_api'
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from '../../locales/zh-CN/warranty_lang.js'
    import langEn from '../../locales/en-US/warranty_lang.js'
    ...
    export default {
        ...
        methods: {
                ...
                this.form.validateFields((err, values) => {
                    ...
                    if (!err) {
                        for (let key in values) {
                            key === 'warrantyCode'&&values[key]!==undefined ? this.queryParam[key] = '%' + values[key].trim() + '%' : this.queryParam[key] = values[key]
                        }
            ...
            addNewTable() { // 新建列表
                this.form.validateFields((err, values) => {
                    values.cycle === 'CURRENT' ? this.$router.replace('/warranty/warranty_add') : this.$message.warning("Only the data of CURRENT cycle can be New");
                })
            },
            editSelectOne() { // 修改选中行
                ...
                if (selectedRowKeyslen == 1) {
                    ...
                    Vue.ls.set('warranty_editSelectKey', editSelectKey)
                    this.$router.replace(`/warranty/warranty_edit`)
          ...
          style: { background: "#FAAD14" }
            ...
            //     console.log(rowKey)
            //     Vue.ls.set('warranty_dblclickRowKey', rowKey)
            //     this.$router.replace('/warranty/warranty_detail')
