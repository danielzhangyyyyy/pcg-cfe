...
<script>
    import {add} from '@api/warrantyCVMapping_api'
                ...
                const taskForm = new Promise((resolve, reject) => {
                    task.form.validateFields((err, values) => {
                        ...
                        for (let key in values) {
                            values[key] !== undefined ? newValues[key] = values[key].trim() : ''
                        }
                        resolve(newValues)
                    })
                })
                    ...
                    add(values[0]).then(res => {
                        if (res.code == 0) {
                            ...
                            setTimeout(() => {
                                ...
                                this.$router.replace(`/warrantyCVMapping/warrantyCVMapping_list`);
                            }, 500);
                        } else {
                            this.$notification.open({
                                message: "Error",
