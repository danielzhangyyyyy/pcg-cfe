...
<script>
    ...
    import {show} from "@api/plantMaintenance_api";
...
export default {
  ...
  created() {
    this.changeLanguage();
    this.id = Vue.ls.get("plantMaintenance_dblclickRowKey");
    console.log(this.id);
  ...
  watch: {
    ...
    $route: {
      handler(route) {
        ...
        if (route.name === "plantMaintenance_detail") {
          this.id = Vue.ls.get("plantMaintenance_dblclickRowKey");
