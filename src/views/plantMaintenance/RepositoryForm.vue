...
<script>
    ...
    import {getDropDownList, show} from "@api/plantMaintenance_api";
  ...
  created() {
    this.changeLanguage();
    this.editSelectKey = Vue.ls.get("plantMaintenance_editSelectKey");
  ...
  watch: {
    ...
    $route: {
      handler(route) {
        ...
        if (route.name === "plantMaintenance_edit") {
          this.editSelectKey = Vue.ls.get("plantMaintenance_editSelectKey");
          console.log(this.editSelectKey);
          this.getDetailfun(this.editSelectKey);
        }
        ...
        .then(res => {
          if (res.code != 0) {
            ...
            setTimeout(() => {
              ...
              this.$router.replace(`/plantMaintenance/plantMaintenance_list`);
            }, 500);
            return;
          }
          console.log(res);
          ...
          for (let i = 0; i < this.detailList.length; i++) {
            if (this.detailList[i].decorator == "plantType") {
              this.detailList[i].dropDownList = [].concat(
                this.plantTypeDropDownList
              );
            } else if (this.detailList[i].decorator === "plantCountry") {
              this.detailList[i].dropDownList = [].concat(
                this.plantCountryRegionDropDownList
              );
            }
          }
          console.log("ininnn", this.detailList);
          // this.$emit('ok', values)
        })
        .catch(er => {
          this.$notification.open({
