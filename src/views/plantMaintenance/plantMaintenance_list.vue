<template>
    <a-card :bordered="false" class="content">
        ...
        <s-table
                ...
                :scroll="scrollSize"
                size="small"
                :rowKey="(record) => `${record.rid +'@'+record.productGroup +'@'+record.plant}`"
                :data="loadData"
                :alert="options.alert"
...
<script>
    ...
    import {del, getDropDownList, list} from "@api/plantMaintenance_api";
    ...
    export default {
        ...
        data() {
            return {
                plantListData: [],
                plantTypeListData: ["DC", "IN_HOUSE", "OEM"],
                filterList: langEn.filter_langEn,
                form: this.$form.createForm(this),
                currentPageManipulationAuth: ["Add", "Delete", "Update", "View"],
                searchLoading: false,
        ...
        watch: {
            ...
            $route: {
                handler(route) {
                    if (route.path === "/plantMaintenance/plantMaintenance_list") {
        ...
        methods: {
            ...
            addNewTable() {
                // 新建列表
                this.$router.replace("/plantMaintenance/plantMaintenance_add");
            },
            editSelectOne() {
                // 修改选中行
                ...
                if (selectedRowKeyslen == 1) {
                    ...
                    Vue.ls.set("plantMaintenance_editSelectKey", editSelectKey);
                    this.$router.replace(`/plantMaintenance/plantMaintenance_edit`);
