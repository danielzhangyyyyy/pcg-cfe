...
<script>
    import {add} from '@api/partSelected_api'
                ...
                const taskForm = new Promise((resolve, reject) => {
                    task.form.validateFields((err, values) => {
                        ...
                        for(let key in values){
                            key !=='productGroup' ? newValue[key] = values[key].trim() : newValue[key] = values[key]
                        }
                        resolve(newValue)
                    })
                })
                    ...
                    add(values[0]).then(res => {
                        if (res.code == 0) {
                            self.$notification.open({
                                message: "Success",
                                description: self.$t("lang.messageAddSuccess"),
                                duration: 6,
                            ...
                            setTimeout(() => {
                                ...
                                this.$router.replace(`/partSelected/partSelected_list`);
                            }, 500);
                        } else {
                            self.$notification.open({
                                message: "Error:",
