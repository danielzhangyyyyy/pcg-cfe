...
<script>
    import Vue from "vue";
    import {update} from "@api/exchangeRate_api";
  ...
  created() {
    this.editSelectKey = Vue.ls.get("exchangeRate_editSelectKey");
  ...
  watch: {
    $route: {
      handler(route) {
        ...
        if (route.name === "exchangeRate_edit") {
          this.editSelectKey = Vue.ls.get("exchangeRate_editSelectKey");
          console.log(this.editSelectKey);
    ...
    validate() {
      ...
      Promise.all([repositoryForm]).then(values => {
        ...
        values[0].rateType = "Rate From US Dollars";
        update(values[0])
          .then(res => {
            if (res.code == 0) {
              ...
              setTimeout(() => {
                ...
                this.$router.replace(`/exchangeRate/exchangeRate_list`);
              }, 500);
            } else {
                 this.$notification.open({
                message: "Error",
