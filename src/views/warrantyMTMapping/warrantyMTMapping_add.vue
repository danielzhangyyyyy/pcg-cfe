...
<script>
    import {add} from '@api/warrantyMTMapping_api'
                ...
                const taskForm = new Promise((resolve, reject) => {
                    task.form.validateFields((err, values) => {
                        ...
                        for (let key in values) {
                            key === 'warrantyCode' ? newValue[key] = values[key].trim() : newValue[key] = values[key]
                        }
                        resolve(newValue)
                    })
                })
                    ...
                    add(values[0]).then(res => {
                        if (res.code == 0) {
                            ...
                            setTimeout(() => {
                                ...
                                this.$router.replace(`/warrantyMTMapping/warrantyMTMapping_list`);
                            }, 500);
                        } else {
                            this.$notification.open({
                                message: "Error",
