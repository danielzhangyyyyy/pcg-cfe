<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_MTYPE_WARR_MAP"
...
<script>
    ...
    import {del, getDropDownList, list} from "@api/warrantyMTMapping_api";
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from "../../locales/zh-CN/warrantyMTMapping_lang.js";
    import langEn from "../../locales/en-US/warrantyMTMapping_lang.js";
  ...
  watch: {
    machineDataList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "machineType") {
          element["dropDownList"] = [].concat(this.machineDataList);
        }
      });
    },
    language(val) {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/warrantyMTMapping/warrantyMTMapping_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
            for(let key in values){
                if(key==='warrantyCode'&&values[key]!==undefined){
                    let newVal = values[key].trim()
                    newVal!==''?this.queryParam[key] = `%${newVal}%` : this.queryParam[key] = values[key]
                } else if(key==='machineType'&&values[key]!==undefined) {
    ...
    getDropDown(param, dropdownList, key) {
      getDropDownList(Object.assign({}, param)).then(res => {
        ...
        res.result.forEach(el => {
          el != null ? dropdownList.push(el[key]) : "";
        });
      });
    },
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/warrantyMTMapping/warrantyMTMapping_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("warrantyMTMapping_editSelectKey", editSelectKey);
        this.$router.replace(`/warrantyMTMapping/warrantyMTMapping_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("warrantyMTMapping_dblclickRowKey", rowKey);
      this.$router.replace("/warrantyMTMapping/warrantyMTMapping_detail");
