<template>
    <a-form @submit="handleSubmit" :form="form" class="form">
        <a-row class="form-row" :gutter="48">
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList.slice(0,6)" :key="item.index">
            ...
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList.slice(6)" :key="item.index">
                <a-form-item :label="item.label">
                    <a-input
...
<script>
    ...
    import {getDropDownList, show} from '@api/gscRealCostGroup_api'
    ...
    export default {
        ...
        created() {
            this.changeLanguage()
            this.editSelectKey = Vue.ls.get('gscRealCostGroup_editSelectKey')
        ...
        watch: {
            ...
            '$route': {
                handler(route) {
                    if (route.name === 'gscRealCostGroup_edit') {
                        this.editSelectKey = Vue.ls.get('gscRealCostGroup_editSelectKey')
                ...
                show(rowKey).then(res => {
                    if (res.code != 0) {
                        ...
                        setTimeout(() => {
                            ...
                            this.$router.replace(`/gscRealCostGroup/gscRealCostGroup_list`);
                        }, 500);
                        return;
                    }
