<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_UI_GROUP_COST_FOR_MODEL"
...
<script>
    ...
    import {del, getDropDownList, getTypeList, list} from "@api/groupCostForModel_api";
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from "../../locales/zh-CN/groupCostForModel_lang.js";
    import langEn from "../../locales/en-US/groupCostForModel_lang.js";
  ...
  watch: {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/groupCostForModel/groupCostForModel_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/groupCostForModel/groupCostForModel_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("groupCostForModel_editSelectKey", editSelectKey);
        this.$router.replace(`/groupCostForModel/groupCostForModel_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("groupCostForModel_dblclickRowKey", rowKey);
      this.$router.replace("/groupCostForModel/groupCostForModel_detail");
