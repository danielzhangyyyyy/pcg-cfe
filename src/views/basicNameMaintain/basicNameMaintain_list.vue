<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      exportId="GET_BASIC_NAME_MAINTAIN"
      uploadId="VAL_BASIC_NAME_MAINTAIN"
    >
      <a-button type="primary" @click="addNewTable" icon="plus">New</a-button>
      <a-button type="primary" @click="deleteSeclectAll" icon="delete">Delete</a-button>
      <a-button type="primary" @click="eventPush" icon="rollback">Return</a-button>
    </tableBtns>
...
import {
  ...
  getSBBTypeSel,
  list
} from "@api/basicNameMaintain_api";
import langZh from "../../locales/zh-CN/basicNameMaintain_lang.js";
import langEn from "../../locales/en-US/basicNameMaintain_lang.js";
...
export default {
  ...
  data() {
    return {
      getProdTypeSelDataList: [],
      getSBBTypeSelDataList: [],
      filterList: langEn.filter_langEn,
      form: this.$form.createForm(this),
      currentPageManipulationAuth: ["UploadEnforced", "ViewEnforced"],
      mdl: {},
      searchLoading: false,
  ...
  created() {
    ...
    this.getListFun();
    this.getDropDown(
      this.getProdTypeSelDataList,
      "productType",
      getProdTypeSel
    );
    this.getDropDown(this.getSBBTypeSelDataList, "sbbType", getSBBTypeSel);
    this.setDropDownLists();
  },
  watch: {
    getProdTypeSelDataList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "productType") {
          element["dropDownList"] = [].concat(this.getProdTypeSelDataList);
        }
      });
    },
    getSBBTypeSelDataList() {
  ...
  methods: {
    eventPush() {
      ...
      setTimeout(() => {
        ...
        this.$router.push({
          path: "/filteredSBBBOMStructure/filteredSBBBomStructure_list"
        });
      }, 500);
    },
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          for (let key in values) {
            if (key === "basicName" && values[key] !== undefined) {
              let val = values.basicName.trim();
              val !== ""
                ? (this.queryParam[key] = `%${val}%`)
                : (this.queryParam[key] = values.basicName);
            } else {
    ...
    getDropDown(dropdownList, key, api) {
      api(Object.assign({})).then(res => {
    ...
    getListFun() {
      ...
      that.loadData = parameter => {
        ...
        if (
          that.queryParam.basicName !== "" ||
          that.queryParam.sbbType !== "" ||
          that.queryParam.productType !== ""
        ) {
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/basicNameMaintain/basicNameMaintain_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("basicNameMaintain_editSelectKey", editSelectKey);
        this.$router.replace(`/basicNameMaintain/basicNameMaintain_edit`);
    ...
    //     console.log(rowKey)
    //     Vue.ls.set('basicNameMaintain_dblclickRowKey', rowKey)
    //     this.$router.replace('/basicNameMaintain/basicNameMaintain_detail')
