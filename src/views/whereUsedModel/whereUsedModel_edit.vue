...
<script>
    import Vue from "vue";
    import {update} from "@api/whereUsedModel_api";
  ...
  created() {
    this.editSelectKey = Vue.ls.get("whereUsedModel_editSelectKey");
  ...
  watch: {
    $route: {
      handler(route) {
        ...
        if (route.name === "whereUsedModel_edit") {
          this.editSelectKey = Vue.ls.get("whereUsedModel_editSelectKey");
          console.log(this.editSelectKey);
          ...
          .then(res => {
            if (res.code == 0) {
              ...
              setTimeout(() => {
                ...
                this.$router.replace(`/whereUsedModel/whereUsedModel_list`);
              }, 500);
            } else {
                 this.$notification.open({
                message: "Error",
