    ...
    <a-card :bordered="false" class="content">
        <a-alert style="margin-bottom: 16px">
            <template slot="message">
                <span :style="{'font-weight': 'bold', 'margin-right': '12px'}">{{description}}</span>
                <a-button
                        type="primary"
                        icon="rollback"
                        @click="goBack"
...
<script>
    ...
    import {detail, getDropDownList} from "@api/netCostBom_api";
    ...
    export default {
        ...
        watch: {
            ...
            $route: {
                handler(to, from) {
                    console.log(to);
                    if (to.path == "/netCostBom/netCostBom_detail") {
                        this.loadData({});
                    }
                },
                deep: true
            ...
            loadData(params) {
                ...
                detail(Object.assign(parameter, self.queryParam)).then(res => {
                    if(res.code == 0){
                        self.description =
                            "Cost Tape For Part: " +
                            self.$route.query.partNumber +
                            ", " +
                            "Plant: " +
                            self.$route.query.plant;
