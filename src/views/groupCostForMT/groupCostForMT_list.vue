<template>
    <a-card :bordered="false" class="content">
        ...
        <tableBtns
                ...
                uploadId="UPLOAD_UI_GROUP_COST_FOR_MT"
...
<script>
    ...
    import {del, getDropDownList, getMachineType, getSelectType, list} from '@api/groupCostForMT_api'
    ...
    export default {
            ...
            bizProdFByBrandDataList() {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "prodFamily") {
                        element["dropDownList"] = [].concat(this.bizProdFByBrandDataList);
                    }
                });
            },
            machineTypeDataList() {
            ...
            groupTypeDataList() {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "type") {
                        element["dropDownList"] = [].concat(this.groupTypeDataList);
                    }
                });
            },
            language(val) {
        ...
        methods: {
                    ...
                    if (!err) {
                        for (let key in values) {
                            if(key === 'owner'){
                                ...
                                val !==''&&values[key]!==undefined?this.queryParam[key] = `%${val}%`:this.queryParam[key] = values[key]
                            } else {
            ...
            getGroupType(params, dropdownList, key, api) {
            ...
            addNewTable() { // 新建列表
                this.$router.replace('/groupCostForMT/groupCostForMT_add')
            },
            editSelectOne() { // 修改选中行
                ...
                if (selectedRowKeyslen == 1) {
                    ...
                    Vue.ls.set('groupCostForMT_editSelectKey', editSelectKey)
                    this.$router.replace(`/groupCostForMT/groupCostForMT_edit`)
          ...
          style: { background: "#FAAD14" }
            ...
            //     console.log(rowKey)
            //     Vue.ls.set('groupCostForMT_dblclickRowKey', rowKey)
            //     this.$router.replace('/groupCostForMT/groupCostForMT_detail')
