    ...
    <a-card :bordered="false" class="content">
        ...
        >
            <template slot="operation" slot-scope="text, record">
                <a href="javascript:;" @click="editSelectOne(record.roleId)">Edit&nbsp;&nbsp;</a>
                <a href="javascript:;" @click="onDelete(record.roleId)">Delete</a>
            </template>
        </a-table>
    </a-card>
</template>
...
<script>
    ...
    import {list, del} from "@api/userRole_api";
    ...
    export default {
        ...
        watch: {
            ...
            $route: {
                handler(route) {
                    if (route.path === "/userRole/userRole_list") {
                        this.handleOk()
                    }
                },
                deep: true
        ...
        methods: {
            ...
            searchOnClick() {
                ...
                this.form.validateFields((err, values) => {
                    ...
                    if (!err) {
                        values.roleName?this.queryParam.roleName = `%${values.roleName.trim()}%`:this.queryParam.roleName = values.roleName
                        this.getListFun({})
                    } else {
            ...
            getListFun(params) {
                ...
                list(Object.assign(parameter, that.queryParam)).then(res => {
                    ...
                    for(let [index,item] of res.result.list.entries()){
                        item.key = index
                    }
            ...
            addNewTable() {
                // 新建列表
                this.$router.replace("/userRole/userRole_add");
            },
            editSelectOne(roleId) {
                Vue.ls.set("userRole_editSelectKey", roleId);
                this.$router.replace(`/userRole/userRole_edit`);
            },
            // 选择列表
