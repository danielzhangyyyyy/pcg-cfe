<template>
    <a-card :bordered="false" class="content">
        ...
        <tableBtns
                ...
                uploadId="UPLOAD_UI_WARRANTY_ID_MAPPING"
...
<script>
    ...
    import {del, getDropDownList, getWarrantyCode, getWarrantyId, list} from '@api/warrantyIdMap_api'
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from '../../locales/zh-CN/warrantyIdMap_lang.js'
    import langEn from '../../locales/en-US/warrantyIdMap_lang.js'
    ...
    export default {
        ...
        watch: {
            ...
            wrapIdDataList(val) {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "basewarranty") {
                        element["dropDownList"] = [].concat(this.wrapIdDataList);
                    }
                });
            },
            wrapCodeDataList(val) {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "warrantyCode") {
                        element["dropDownList"] = [].concat(this.wrapCodeDataList);
                    }
                });
            },
            language(val) {
            ...
            '$route': {
                handler(route) {
                    ...
                    if (route.path === '/warrantyIdMap/warrantyIdMap_list') {
                        this.handleOk()
                    }
                },
                deep: true
        ...
        methods: {
            ...
            getWarpId(dropdownList, key) {
                getWarrantyId({}).then(res => {
            ...
            getWarrantyCode(dropdownList, key) {
                getWarrantyCode({}).then(res => {
                    dropdownList.splice(0);
                    res.result.forEach(el => {
                        dropdownList.push(el[key]);
                    });
            ...
            addNewTable() { // 新建列表
                this.$router.replace('/warrantyIdMap/warrantyIdMap_add')
            },
            editSelectOne() { // 修改选中行
                ...
                if (selectedRowKeyslen == 1) {
                    ...
                    Vue.ls.set('warrantyIdMap_editSelectKey', editSelectKey)
                    this.$router.replace(`/warrantyIdMap/warrantyIdMap_edit`)
          ...
          style: { background: "#FAAD14" }
            ...
            doubleClickOnRow(record) { // 双击行查看详情
                ...
                console.log(rowKey)
                Vue.ls.set('warrantyIdMap_dblclickRowKey', rowKey)
                this.$router.replace('/warrantyIdMap/warrantyIdMap_detail')
