...
<script>
    ...
    import {del, getDropDownList, list} from '@api/commitHigh_api'
        ...
        watch: {
            ...
            '$route': {
                handler(route) {
                    ...
                    if (route.path === '/commitHigh/commitHigh_list') {
                        this.handleOk()
                    }
                },
                deep: true
        ...
        methods: {
            ...
            addNewTable() { // 新建列表
                this.form.validateFields((err, values) => {
                    // console.log("err", err);
                    console.log("values", values);
                    if(values.cycle === 'CURRENT') {
                        this.$router.replace('/commitHigh/commitHigh_add')
                    } else {
            ...
            editSelectOne() { // 修改选中行
                ...
                if (selectedRowKeyslen == 1) {
                    ...
                    console.log(this.selectedRows, '11111111editSelectKey')
                    Vue.ls.set('commitHigh_editSelectKey', editSelectKey)
                    for (let item of this.selectedRows) {
                        if(item.cycle === 'CURRENT') {
                            this.$router.replace(`/commitHigh/commitHigh_edit`)
                        } else {
                            this.$notification.open({
                                message: "Warn",
            ...
            //     console.log(rowKey)
            //     Vue.ls.set('commitHigh_dblclickRowKey', rowKey)
            //     this.$router.replace('/commitHigh/commitHigh_detail')
