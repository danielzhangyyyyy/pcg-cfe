<template>
    <a-form @submit="handleSubmit" :form="form" class="form">
        <a-row class="form-row" :gutter="48">
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList.slice(0,5)" :key="item.index">
            ...
            <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in detailList.slice(5)" :key="item.index">
                <a-form-item :label="item.label">
                    <a-input
                            :placeholder="item.placeHolder"
                            v-if="item.inputType=='input'"
...
<script>
    ...
    import Vue from 'vue'
    import langZh from '../../locales/zh-CN/countryAdjustPH_lang.js'
    import langEn from '../../locales/en-US/countryAdjustPH_lang.js'
    import {getDropDownList, show} from '@api/countryAdjustPH_api'
    ...
    export default {
        ...
        created() {
            this.changeLanguage()
            this.editSelectKey = Vue.ls.get('countryAdjustPH_editSelectKey')
        ...
        watch: {
            ...
            '$route': {
                handler(route) {
                    ...
                    if (route.name === 'countryAdjustPH_edit') {
                        this.editSelectKey = Vue.ls.get('countryAdjustPH_editSelectKey')
                        this.getDetailfun(this.editSelectKey)
                    }
                },
                ...
                show(rowKey).then(res => {
                    if (res.code != 0) {
                        ...
                        setTimeout(() => {
                            ...
                            this.$router.replace(`/countryAdjustPH/countryAdjustPH_list`);
                        }, 500);
                        return;
                    }
