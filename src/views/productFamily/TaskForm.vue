      ...
      <a-col :lg="8" :md="12" :sm="24" v-for="(item,index) in babelList" :key="index">
        <a-form-item :label="item.label">
          ...
          <a-auto-complete
            ...
            :placeholder="item.message"
            :allowClear="item.allowClear"
            :dataSource="costProdFamilyDataList"
            :filterOption="filterOption"
            v-decorator="[item.decorator, { rules: [{required: item.required, message: item.message }]}]"
          />
          <a-date-picker
...
<script>
    ...
    import Vue from "vue";
    import {getDropDownList} from "@api/productFamily_api";
  ...
  activated() {
    ...
    handleDropDownChange(value, decorator) {
      ...
      } else if (decorator == "costProdFamily") {
        this.form.setFieldsValue({ ["costProdFamily"]: value });
      }
    },
    // 更改列表国际化
    changeLanguage() {
      ...
      if (this.language == "zh-CN") {
        this.babelList = langZh.add_langZh;
        // eslint-disable-next-line
      } else if (this.language == "en-US") {
        this.babelList = langEn.add_langEn;
      }
      // this.babelList.forEach(element => {
      //   if (element["decorator"] == "costProdFamily") {
      //     this.selectDisable = false
      //     this.inputDisable = element["editDisabled"]
      //   }
      //   console.log("selectInputStatus",this.selectInputStatus);
      // });
    },
    handleSubmit(e) {
      e.preventDefault();
