...
<script>
    ...
    import {update} from "@api/productFamily_api";
  ...
  created() {
    this.editSelectKey = Vue.ls.get("productFamily_editSelectKey");
  ...
  watch: {
    $route: {
      handler(route) {
        ...
        if (route.name === "productFamily_edit") {
          this.editSelectKey = Vue.ls.get("productFamily_editSelectKey");
          console.log(this.editSelectKey);
    ...
    validate() {
      ...
      Promise.all([repositoryForm]).then(values => {
        values[0].rid = this.editSelectKey;
        values[0].eolStartdate = moment(values[0].eolStartdate).format(
          "YYYY-MM-DD"
        );
        values[0].eolEnddate = moment(values[0].eolEnddate).format(
          "YYYY-MM-DD"
        );
        console.log(values);
        this.loading = true
