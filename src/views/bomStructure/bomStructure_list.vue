    ...
    <a-alert style="margin-bottom: 16px">
      <template slot="message">
        <div style="margin-right: 12px">
          <span v-if="!alterDescription" :style="{'font-weight': 'bold'}">BOM Structure</span>
          <span v-else>
...
export default {
  ...
  methods: {
    ...
    getShow() {
      ...
      alterShow(
        Object.assign({}, { item: this.assemble, itemType: "ANY" })
      ).then(res => {
        this.alterDescription = res.result.itemDesc;
    ...
    getListFun() {
      ...
      list(Object.assign({}, self.queryParam)).then(res => {
        ...
        for (let i = 0; i < res.result.bomStructure.length; i++) {
          ...
          res.result.bomStructure[i].subItems && self.defaultExpandAllRows
            ? self.expandedRowKeys.push(
          ...
          if (res.result.bomStructure[i].subItems) {
            for (
              let j = 0;
              j < res.result.bomStructure[i].subItems.length;
              j++
            ) {
              res.result.bomStructure[i].subItems[j].key =
                100 * (i + 1) + (j + 1).toString();
            }
          }
          res.result.bomStructure[i].entityDto.children =
            res.result.bomStructure[i].subItems;
