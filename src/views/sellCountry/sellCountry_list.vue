<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_UI_SELLABLE_COUNTRY"
...
<script>
    ...
    import {del, getDropDownList, list} from "@api/sellCountry_api";
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from "../../locales/zh-CN/sellCountry_lang.js";
    import langEn from "../../locales/en-US/sellCountry_lang.js";
  ...
  data() {
    return {
      currentPageManipulationAuth: ["View", "Upload", "Delete", "Add"],
      subgeoDropdownList: [],
      countryDropdownList: [],
  ...
  watch: {
    ...
    countryDropdownList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "country") {
          element["dropDownList"] = [].concat(this.countryDropdownList);
        }
      });
    },
    language(val) {
    ...
    $route: {
      handler(route) {
        ...
        if (route.path === "/sellCountry/sellCountry_list") {
          this.handleOk();
        }
      },
      deep: true
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/sellCountry/sellCountry_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("sellCountry_editSelectKey", editSelectKey);
        this.$router.replace(`/sellCountry/sellCountry_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("sellCountry_dblclickRowKey", rowKey);
      this.$router.replace("/sellCountry/sellCountry_detail");
