...
<script>
    ...
    import {del, getDropDownList, list} from '@api/modelWeight_api'
    ...
    export default {
        ...
        watch: {
            ...
            $route: {
                handler(route) {
                    if (route.path === '/modelWeight/modelWeight_list') {
        ...
        methods: {
            ...
            addNewTable() { // 新建列表
                this.$router.replace('/modelWeight/modelWeight_add')
            },
            editSelectOne() { // 修改选中行
                const selectedRowKeyslen = this.selectedRowKeys.length
                if (selectedRowKeyslen == 1) {
                    const editSelectKey = this.selectedRowKeys[0]
                    Vue.ls.set('modelWeight_editSelectKey', editSelectKey)
                    this.$router.replace(`/modelWeight/modelWeight_edit`)
