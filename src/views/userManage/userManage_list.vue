<template>
    <a-card :bordered="false" class="content">
        ...
        <tableBtns
                ...
                exportId="EXPORT_USER_INFO"
                @addNewTable="addNewTable"
                @editSelectOne="editSelectOne"
                @deleteSeclectAll="deleteSeclectAll"
        >
            <a-button type="primary" @click="eventChangePassword">Reset Password</a-button>
        </tableBtns>
...
<script>
    ...
    import {list, getUserType, del, changePassword} from "@api/userManage_api";
    ...
    export default {
        ...
        data() {
            return {
                ...
                currentPageManipulationAuth: ["View", 'Add', 'Update', 'Delete', 'Export'],
                searchLoading: false,
        ...
        watch: {
            ...
            $route: {
                handler(route) {
                    if (route.path === "/userManage/userManage_list") {
                        this.handleOk()
                    }
                },
                deep: true
        ...
        methods: {
            eventChangePassword(){
                ...
                if (this.selectedRowKeys.length == 0) {
                    ...
                    if (this.selectedRows[0].userType == 'External' || this.selectedRows[0].userType == 'Monitor') {
                        changePassword({itcode: this.selectedRows[0].itcode}).then(res => {
                            console.log(res.result, 333)
                            if (res.code == 0) {
                                self.$notification.open({
                                    message: "Success",
                                    description: 'Password has been reset to pw!',
                                    duration: 6,
                        ...
                        self.$notification.open({
                            message: "Warn",
                            description: 'Only external user can reset the password!',
                            duration: 6,
                            style: {background: "#FAAD14"}
                        });
                    }
            ...
            searchOnClick() {
                ...
                this.form.validateFields((err, values) => {
                    ...
                    if (!err) {
                        ...
                        values.firstName ? this.queryParam.firstName = `%${values.firstName.trim()}%`:this.queryParam.firstName = values.firstName
                        values.lastName ? this.queryParam.lastName = `%${values.lastName.trim()}%`:this.queryParam.lastName = values.lastName
                        this.queryParam.userType = values.userType
            ...
            addNewTable() {
                // 新建列表
                this.$router.replace("/userManage/userManage_add");
            },
            editSelectOne() {
                // 修改选中行
                const selectedRowKeyslen = this.selectedRowKeys.length;
                if (selectedRowKeyslen == 1) {
                    console.log(this.selectedRows, 'selectedRows111111')
                    const editSelectKey = this.selectedRows[0].itcode;
                    Vue.ls.set("userManageUpload_editSelectKey", editSelectKey);
                    this.$router.replace(`/userManage/userManage_edit`);
            ...
            resetSearchForm() {
                ...
                for(let item in this.queryParam) item = ''
            }
        }
    };
</script>
