<template>
    <a-card :bordered="false" class="content">
        ...
        <tableBtns
                ...
                uploadId="UPLOAD_SBB_BOM_STRU_REPL"
...
<script>
    ...
    import {del, getBomType, getCostPlantList, getDropDownList, getPlantList, list} from '@api/sBBBOMStructureRepl_api'
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from '../../locales/zh-CN/sBBBOMStructureRepl_lang.js'
    import langEn from '../../locales/en-US/sBBBOMStructureRepl_lang.js'
        ...
        watch: {
            bomTypeDataList(val) {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "bomType") {
                        element["dropDownList"] = [].concat(this.bomTypeDataList);
                    }
                });
            },
            plantDataList(val) {
            ...
            costPlantDataList(val) {
                this.filterList.forEach(element => {
                    if (element["decorator"] == "costPlant") {
                        element["dropDownList"] = [].concat(this.costPlantDataList);
                    }
                });
            },
            language(val) {
            ...
            '$route': {
                handler(route) {
                    ...
                    if (route.path === '/sBBBOMStructureRepl/sBBBOMStructureRepl_list') {
                        this.handleOk()
                    }
                },
                deep: true
        ...
        methods: {
            ...
            searchOnClick() {
                ...
                this.form.validateFields((err, values) => {
                    ...
                    console.log("values", values.sbb == '');
                    if (!err) {
                        for (let key in values) {
                            if ((key === 'sbb' || key === 'component') && values[key] !== undefined) {
                                let newValues = values[key].trim()
                                newValues !== '' ? this.queryParam[key] = `%${newValues}%` : this.queryParam[key] = values[key]
                            } else {
            ...
            getListFun() {
                ...
                that.loadData = parameter => {
                    if (!this.queryParam.hasOwnProperty('bomType')) {
            ...
            deleteSeclectAll() {
                ...
                this.selectedRows.forEach(element => {
                    rowKeys.push({
                        rid: element.rid,
                        bomType: element.bomType,
                        plant: element.plant,
                        costPlant: element.costPlant,
                        sbb: element.sbb,
