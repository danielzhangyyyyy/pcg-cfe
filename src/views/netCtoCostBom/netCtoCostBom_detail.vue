        ...
        >
          <span slot="sbb" slot-scope="text, record">
            <a @click="partOnClick(record, text)" :class="{visitedPart: visitedKey.indexOf(text) != -1}">
               <strong v-if="record.isCosttape === 'Y'"> {{ text }}</strong>
...
<script>
    import {detail, getDropDownList, list} from "@api/netCtoCostBom_api";
    ...
    export default {
        ...
        watch: {
            ...
            '$route': {
                handler(route) {
                    if (route.path === "/netCtoCostBom/netCtoCostBom_detail") {
                        this.loadData({});
                    }
                },
                deep: true
        ...
        methods: {
            partOnClick(record, text) {
                ...
                setTimeout(() => {
                    this.$store.dispatch(
                        "ToggleCloseTab",
                        "/netCtoCostBom/netCtoCostBom_costTape" + this.lastQuery
                    );
                    ...
                    this.$router.replace({
                        path: "/netCtoCostBom/netCtoCostBom_costTape",
                        query: {
                            cycle: record.cycle,
                            mfgPlant: record.costPlant,
                            partNumber: text
            ...
            goBack() {
                ...
                setTimeout(() => {
                    ...
                    this.$router.replace(`/netCtoCostBom/netCtoCostBom_list`);
                }, 500);
            },
