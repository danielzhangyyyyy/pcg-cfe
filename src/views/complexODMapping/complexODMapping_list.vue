<template>
  <a-card :bordered="false" class="content">
    ...
    <tableBtns
      ...
      uploadId="UPLOAD_U_COMPLEX_OD_CVSBB_MAPPING"
      :queryParamemter="queryParam"
    ></tableBtns>
    <!--:queryParamemter="queryParam" -->
    <!--导出表没有filter中的字段，传到后台会报错-->
...
<script>
    ...
    import {del, getDropDownList, list} from "@api/complexODMapping_api";
    import { getXScrollSize } from "@api/publicFunc_api";
    import langZh from "../../locales/zh-CN/complexODMapping_lang.js";
    import langEn from "../../locales/en-US/complexODMapping_lang.js";
  ...
  created() {
    ...
    /*   this.getDropDown(
      { moduleName: "getProductFamilyByBrand" },
      this.aspPrdFamilyDropDownList,
      "COST_PROD_FAMILY"
    ); */
  ...
  watch: {
    ...
    aspPrdFamilyDropDownList(val) {
      this.filterList.forEach(element => {
        if (element["decorator"] == "aspPrdFamily") {
          element["dropDownList"] = [].concat(this.aspPrdFamilyDropDownList);
        }
      });
    },
    $route: {
      handler(route) {
        ...
        if (route.path === "/complexODMapping/complexODMapping_list") {
          this.handleOk();
        }
      },
      deep: true
  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          this.$notification["error"]({
            message: "Search condition error:",
            description:
              "please input search conditions with correct combination."
          });
        }
      });
    },
    setDropDownLists() {
      this.filterList.forEach(element => {
        if (element["decorator"] == "brand") {
          element["dropDownList"] = [].concat(this.brandDropdownList);
        } else if (element["decorator"] == "aspPrdFamily") {
          element["dropDownList"] = [].concat(this.aspPrdFamilyDropDownList);
        }
      });
    },
        ...
        if (this.initial) {
          ...
          return list(Object.assign(parameter, that.queryParam)).then(res => {
            ...
            if (res.code == 0) {
              ...
              this.$message.warning("Search result error");
            }
            that.searchLoading = false;
    ...
    addNewTable() {
      // 新建列表
      this.$router.replace("/complexODMapping/complexODMapping_add");
    },
    editSelectOne() {
      // 修改选中行
      ...
      if (selectedRowKeyslen == 1) {
        ...
        Vue.ls.set("complexODMapping_editSelectKey", editSelectKey);
        this.$router.replace(`/complexODMapping/complexODMapping_edit`);
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("complexODMapping_dblclickRowKey", rowKey);
      this.$router.replace("/complexODMapping/complexODMapping_detail");
