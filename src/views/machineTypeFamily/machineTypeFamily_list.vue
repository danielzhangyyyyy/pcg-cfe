...
export default {
  ...
  created() {
    ...
    getProductFamilyDropDownList({}).then(res => {
      this.ProductFamilyDropdownList.length = 0;
      res.result.forEach(el => {
        this.ProductFamilyDropdownList.push(el["prodFamily"]);
      });
    });
    getProductTypeDropDownList({})
      .then(res => {
  ...
  methods: {
    ...
    doubleClickOnRow(record) {
      ...
      console.log(rowKey);
      Vue.ls.set("machineTypeFamily_dblclickRowKey", rowKey);
      this.$router.replace("/machineTypeFamily/machineTypeFamily_detail");
