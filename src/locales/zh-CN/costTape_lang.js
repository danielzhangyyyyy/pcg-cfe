// list页面表头
const tableTitle_langZh = [
    {
        fixed: 'left',
    },
    {
        title: 'CYCLE',
        dataIndex: 'cycle',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PRODUCT_GROUP',
        dataIndex: 'productGroup',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PART_NUMBER',
        dataIndex: 'partNumber',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PLANT',
        dataIndex: 'plant',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'COST_ELEM',
        dataIndex: 'costElem',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PG',
        dataIndex: 'pg',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SPEC',
        dataIndex: 'spec',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'COMMODITY',
        dataIndex: 'commodity',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SOURCE',
        dataIndex: 'source',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'USED',
        dataIndex: 'used',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'DESCRIPTION',
        dataIndex: 'description',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CURRENCY_NAME',
        dataIndex: 'currencyName',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'm1',
        dataIndex: 'm1',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm2',
        dataIndex: 'm2',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm3',
        dataIndex: 'm3',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm4',
        dataIndex: 'm4',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm5',
        dataIndex: 'm5',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm6',
        dataIndex: 'm6',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm7',
        dataIndex: 'm7',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm8',
        dataIndex: 'm8',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm9',
        dataIndex: 'm9',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M10',
        dataIndex: 'm10',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M11',
        dataIndex: 'm11',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M12',
        dataIndex: 'm12',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'NEW_PART_NUMBER',
        dataIndex: 'newPartNumber',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'OWNER_COSTTAPE',
        dataIndex: 'ownerCosttape',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'LAST_MODIFIED_DATE_COSTTAPE',
        dataIndex: 'lastModifiedDateCosttape',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CREATED_DATE_COSTTAPE',
        dataIndex: 'createdDateCosttape',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'COMPANY_CONFIDENTIAL',
        dataIndex: 'companyConfidential',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'LANDED',
        dataIndex: 'landed',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'COST_PERIODS',
        dataIndex: 'costPeriods',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'ACTION',
        dataIndex: 'action',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'STATUS',
        dataIndex: 'status',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_CREATED_DATE',
        dataIndex: 'sysCreatedDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_CREATED_BY',
        dataIndex: 'sysCreatedBy',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_LAST_MODIFIED_DATE',
        dataIndex: 'sysLastModifiedDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_LAST_MODIFIED_BY',
        dataIndex: 'sysLastModifiedBy',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PART_TYPE',
        dataIndex: 'partType',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CHANGE_ID',
        dataIndex: 'changeId',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CHANGE_ACTION',
        dataIndex: 'changeAction',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'WEIGHTED_AVERAGE',
        dataIndex: 'weightedAverage',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'LATTEST_PROCUR',
        dataIndex: 'lattestProcur',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'UPLOAD_DATE',
        dataIndex: 'uploadDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'UPLOAD_BY',
        dataIndex: 'uploadBy',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'IMPORT_DATE',
        dataIndex: 'importDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'IMPORT_BY',
        dataIndex: 'importBy',
        align: 'left',
        width: 300,
        sorter: true
    },
]

//edit页面
const editAndDetail_langZh =[
    {
        label: 'CYCLE',
        decorator: 'cycle',
        required: true,
        message: '请输入CYCLE'
    },
    {
        label: 'PRODUCT_GROUP',
        decorator: 'productGroup',
        required: true,
        message: '请输入PRODUCT_GROUP'
    },
    {
        label: 'PART_NUMBER',
        decorator: 'partNumber',
        required: true,
        message: '请输入PART_NUMBER'
    },
    {
        label: 'PLANT',
        decorator: 'plant',
        required: true,
        message: '请输入PLANT'
    },
    {
        label: 'COST_ELEM',
        decorator: 'costElem',
        required: true,
        message: '请输入COST_ELEM'
    },
    {
        label: 'PG',
        decorator: 'pg',
        required: true,
        message: '请输入PG'
    },
    {
        label: 'SPEC',
        decorator: 'spec',
        required: true,
        message: '请输入SPEC'
    },
    {
        label: 'COMMODITY',
        decorator: 'commodity',
        required: true,
        message: '请输入COMMODITY'
    },
    {
        label: 'SOURCE',
        decorator: 'source',
        required: true,
        message: '请输入SOURCE'
    },
    {
        label: 'USED',
        decorator: 'used',
        required: true,
        message: '请输入USED'
    },
    {
        label: 'DESCRIPTION',
        decorator: 'description',
        required: true,
        message: '请输入DESCRIPTION'
    },
    {
        label: 'CURRENCY_NAME',
        decorator: 'currencyName',
        required: true,
        message: '请输入CURRENCY_NAME'
    },
    {
        label: 'M1',
        decorator: 'm1',
        required: true,
        message: '请输入M1'
    },
    {
        label: 'M2',
        decorator: 'm2',
        required: true,
        message: '请输入M2'
    },
    {
        label: 'M3',
        decorator: 'm3',
        required: true,
        message: '请输入M3'
    },
    {
        label: 'M4',
        decorator: 'm4',
        required: true,
        message: '请输入M4'
    },
    {
        label: 'M5',
        decorator: 'm5',
        required: true,
        message: '请输入M5'
    },
    {
        label: 'M6',
        decorator: 'm6',
        required: true,
        message: '请输入M6'
    },
    {
        label: 'M7',
        decorator: 'm7',
        required: true,
        message: '请输入M7'
    },
    {
        label: 'M8',
        decorator: 'm8',
        required: true,
        message: '请输入M8'
    },
    {
        label: 'M9',
        decorator: 'm9',
        required: true,
        message: '请输入M9'
    },
    {
        label: 'M10',
        decorator: 'm10',
        required: true,
        message: '请输入M10'
    },
    {
        label: 'M11',
        decorator: 'm11',
        required: true,
        message: '请输入M11'
    },
    {
        label: 'M12',
        decorator: 'm12',
        required: true,
        message: '请输入M12'
    },
    {
        label: 'NEW_PART_NUMBER',
        decorator: 'newPartNumber',
        required: true,
        message: '请输入NEW_PART_NUMBER'
    },
    {
        label: 'OWNER_COSTTAPE',
        decorator: 'ownerCosttape',
        required: true,
        message: '请输入OWNER_COSTTAPE'
    },
    {
        label: 'LAST_MODIFIED_DATE_COSTTAPE',
        decorator: 'lastModifiedDateCosttape',
        required: true,
        message: '请输入LAST_MODIFIED_DATE_COSTTAPE'
    },
    {
        label: 'CREATED_DATE_COSTTAPE',
        decorator: 'createdDateCosttape',
        required: true,
        message: '请输入CREATED_DATE_COSTTAPE'
    },
    {
        label: 'COMPANY_CONFIDENTIAL',
        decorator: 'companyConfidential',
        required: true,
        message: '请输入COMPANY_CONFIDENTIAL'
    },
    {
        label: 'LANDED',
        decorator: 'landed',
        required: true,
        message: '请输入LANDED'
    },
    {
        label: 'COST_PERIODS',
        decorator: 'costPeriods',
        required: true,
        message: '请输入COST_PERIODS'
    },
    {
        label: 'ACTION',
        decorator: 'action',
        required: true,
        message: '请输入ACTION'
    },
    {
        label: 'STATUS',
        decorator: 'status',
        required: true,
        message: '请输入STATUS'
    },
    {
        label: 'SYS_CREATED_DATE',
        decorator: 'sysCreatedDate',
        required: true,
        message: '请输入SYS_CREATED_DATE'
    },
    {
        label: 'SYS_CREATED_BY',
        decorator: 'sysCreatedBy',
        required: true,
        message: '请输入SYS_CREATED_BY'
    },
    {
        label: 'SYS_LAST_MODIFIED_DATE',
        decorator: 'sysLastModifiedDate',
        required: true,
        message: '请输入SYS_LAST_MODIFIED_DATE'
    },
    {
        label: 'SYS_LAST_MODIFIED_BY',
        decorator: 'sysLastModifiedBy',
        required: true,
        message: '请输入SYS_LAST_MODIFIED_BY'
    },
    {
        label: 'PART_TYPE',
        decorator: 'partType',
        required: true,
        message: '请输入PART_TYPE'
    },
    {
        label: 'CHANGE_ID',
        decorator: 'changeId',
        required: true,
        message: '请输入CHANGE_ID'
    },
    {
        label: 'CHANGE_ACTION',
        decorator: 'changeAction',
        required: true,
        message: '请输入CHANGE_ACTION'
    },
    {
        label: 'WEIGHTED_AVERAGE',
        decorator: 'weightedAverage',
        required: true,
        message: '请输入WEIGHTED_AVERAGE'
    },
    {
        label: 'LATTEST_PROCUR',
        decorator: 'lattestProcur',
        required: true,
        message: '请输入LATTEST_PROCUR'
    },
    {
        label: 'UPLOAD_DATE',
        decorator: 'uploadDate',
        required: true,
        message: '请输入UPLOAD_DATE'
    },
    {
        label: 'UPLOAD_BY',
        decorator: 'uploadBy',
        required: true,
        message: '请输入UPLOAD_BY'
    },
    {
        label: 'IMPORT_DATE',
        decorator: 'importDate',
        required: true,
        message: '请输入IMPORT_DATE'
    },
    {
        label: 'IMPORT_BY',
        decorator: 'importBy',
        required: true,
        message: '请输入IMPORT_BY'
    },
]

export default {
    tableTitle_langZh,
    editAndDetail_langZh
}