// list页面表头
const tableTitle_langZh = [
    {
        fixed: 'left',
    },
    {
        title: 'CYCLE',
        dataIndex: 'cycle',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PRODUCT_GROUP',
        dataIndex: 'productGroup',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'BRAND',
        dataIndex: 'brand',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PLANT',
        dataIndex: 'plant',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PROD_FAMILY',
        dataIndex: 'prodFamily',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'MACHINE_TYPE',
        dataIndex: 'machineType',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'GEO_TYPE',
        dataIndex: 'geoType',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'GEO',
        dataIndex: 'geo',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'FREIGHT_TYPE',
        dataIndex: 'freightType',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'GL_PERCENTAGE',
        dataIndex: 'glPercentage',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'GEO_PERCENTAGE',
        dataIndex: 'geoPercentage',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'GEO_UPDATE_FLAG',
        dataIndex: 'geoUpdateFlag',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CURRENCY',
        dataIndex: 'currency',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'PROD_H',
        dataIndex: 'prodH',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'm1',
        dataIndex: 'm1',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm2',
        dataIndex: 'm2',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm3',
        dataIndex: 'm3',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm4',
        dataIndex: 'm4',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm5',
        dataIndex: 'm5',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm6',
        dataIndex: 'm6',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm7',
        dataIndex: 'm7',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm8',
        dataIndex: 'm8',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'm9',
        dataIndex: 'm9',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M10',
        dataIndex: 'm10',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M11',
        dataIndex: 'm11',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'M12',
        dataIndex: 'm12',
        align: 'right',
        width: 300,
        sorter: true
    },
    {
        title: 'ACTION',
        dataIndex: 'action',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'STATUS',
        dataIndex: 'status',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_CREATED_DATE',
        dataIndex: 'sysCreatedDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_CREATED_BY',
        dataIndex: 'sysCreatedBy',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_LAST_MODIFIED_DATE',
        dataIndex: 'sysLastModifiedDate',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'SYS_LAST_MODIFIED_BY',
        dataIndex: 'sysLastModifiedBy',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CHANGE_ID',
        dataIndex: 'changeId',
        align: 'left',
        width: 300,
        sorter: true
    },
    {
        title: 'CHANGE_ACTION',
        dataIndex: 'changeAction',
        align: 'left',
        width: 300,
        sorter: true
    },
]

//edit页面
const editAndDetail_langZh =[
    {
        label: 'CYCLE',
        decorator: 'cycle',
        required: true,
        message: '请输入CYCLE'
    },
    {
        label: 'PRODUCT_GROUP',
        decorator: 'productGroup',
        required: true,
        message: '请输入PRODUCT_GROUP'
    },
    {
        label: 'BRAND',
        decorator: 'brand',
        required: true,
        message: '请输入BRAND'
    },
    {
        label: 'PLANT',
        decorator: 'plant',
        required: true,
        message: '请输入PLANT'
    },
    {
        label: 'PROD_FAMILY',
        decorator: 'prodFamily',
        required: true,
        message: '请输入PROD_FAMILY'
    },
    {
        label: 'MACHINE_TYPE',
        decorator: 'machineType',
        required: true,
        message: '请输入MACHINE_TYPE'
    },
    {
        label: 'GEO_TYPE',
        decorator: 'geoType',
        required: true,
        message: '请输入GEO_TYPE'
    },
    {
        label: 'GEO',
        decorator: 'geo',
        required: true,
        message: '请输入GEO'
    },
    {
        label: 'FREIGHT_TYPE',
        decorator: 'freightType',
        required: true,
        message: '请输入FREIGHT_TYPE'
    },
    {
        label: 'GL_PERCENTAGE',
        decorator: 'glPercentage',
        required: true,
        message: '请输入GL_PERCENTAGE'
    },
    {
        label: 'GEO_PERCENTAGE',
        decorator: 'geoPercentage',
        required: true,
        message: '请输入GEO_PERCENTAGE'
    },
    {
        label: 'GEO_UPDATE_FLAG',
        decorator: 'geoUpdateFlag',
        required: true,
        message: '请输入GEO_UPDATE_FLAG'
    },
    {
        label: 'CURRENCY',
        decorator: 'currency',
        required: true,
        message: '请输入CURRENCY'
    },
    {
        label: 'PROD_H',
        decorator: 'prodH',
        required: true,
        message: '请输入PROD_H'
    },
    {
        label: 'M1',
        decorator: 'm1',
        required: true,
        message: '请输入M1'
    },
    {
        label: 'M2',
        decorator: 'm2',
        required: true,
        message: '请输入M2'
    },
    {
        label: 'M3',
        decorator: 'm3',
        required: true,
        message: '请输入M3'
    },
    {
        label: 'M4',
        decorator: 'm4',
        required: true,
        message: '请输入M4'
    },
    {
        label: 'M5',
        decorator: 'm5',
        required: true,
        message: '请输入M5'
    },
    {
        label: 'M6',
        decorator: 'm6',
        required: true,
        message: '请输入M6'
    },
    {
        label: 'M7',
        decorator: 'm7',
        required: true,
        message: '请输入M7'
    },
    {
        label: 'M8',
        decorator: 'm8',
        required: true,
        message: '请输入M8'
    },
    {
        label: 'M9',
        decorator: 'm9',
        required: true,
        message: '请输入M9'
    },
    {
        label: 'M10',
        decorator: 'm10',
        required: true,
        message: '请输入M10'
    },
    {
        label: 'M11',
        decorator: 'm11',
        required: true,
        message: '请输入M11'
    },
    {
        label: 'M12',
        decorator: 'm12',
        required: true,
        message: '请输入M12'
    },
    {
        label: 'ACTION',
        decorator: 'action',
        required: true,
        message: '请输入ACTION'
    },
    {
        label: 'STATUS',
        decorator: 'status',
        required: true,
        message: '请输入STATUS'
    },
    {
        label: 'SYS_CREATED_DATE',
        decorator: 'sysCreatedDate',
        required: true,
        message: '请输入SYS_CREATED_DATE'
    },
    {
        label: 'SYS_CREATED_BY',
        decorator: 'sysCreatedBy',
        required: true,
        message: '请输入SYS_CREATED_BY'
    },
    {
        label: 'SYS_LAST_MODIFIED_DATE',
        decorator: 'sysLastModifiedDate',
        required: true,
        message: '请输入SYS_LAST_MODIFIED_DATE'
    },
    {
        label: 'SYS_LAST_MODIFIED_BY',
        decorator: 'sysLastModifiedBy',
        required: true,
        message: '请输入SYS_LAST_MODIFIED_BY'
    },
    {
        label: 'CHANGE_ID',
        decorator: 'changeId',
        required: true,
        message: '请输入CHANGE_ID'
    },
    {
        label: 'CHANGE_ACTION',
        decorator: 'changeAction',
        required: true,
        message: '请输入CHANGE_ACTION'
    },
]

export default {
    tableTitle_langZh,
    editAndDetail_langZh
}